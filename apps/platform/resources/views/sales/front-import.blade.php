...
@section('content')
    <section class="panel page-header">
        ...
        <p>This page shows how a Front POS sale affects WooCommerce stock. Creating a Woo order is optional.</p>
        <div class="notice">Default behavior is stock-only. A Woo order is only created when an admin manually clicks the order button.</div>
    ...
    <section class="status-board">
        ...
        <div class="status-card {{ $import->order_import_status === 'imported' ? 'ready' : 'warning' }}">
            ...
            <strong>{{ ucfirst(str_replace('_', ' ', $import->order_import_status)) }}</strong>
            <span>{{ $import->orderMapping?->woo_order_id ? 'Woo order '.$import->orderMapping->woo_order_id : 'Not created unless admin chooses it' }}</span>
        </div>
    </section>
    ...
    <section class="panel">
        <div class="split-row">
            <div>
                <h2>Stock action</h2>
                <p class="muted">This reduces WooCommerce stock by the quantities sold in Front. It does not create a Woo order.</p>
            </div>
            @if (in_array($import->stock_status, ['pending', 'failed', 'blocked'], true))
                <form method="post" action="{{ route('front-sales.adjust-stock', $import) }}">
                    @csrf
                    <button type="submit">Adjust Woo stock</button>
                </form>
            @else
